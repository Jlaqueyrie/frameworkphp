...
class User{
    ...
    public function getUserBiography($i_id){
        $this->h_db->query('SELECT description FROM users WHERE id=:sql_req_id');
        $this->h_db->bind(':sql_req_id',$i_id);
        $result = $this->h_db->getSingleData();
        if($result){
    ...
    public function getUserNameById($id){
        $this->h_db->query('SELECT * FROM users WHERE id=:id');
        $this->h_db->bind(':id', $id);
        $row = $this->h_db->getSingleData();
    ...
    public function getUserAvatarById($id){
        ...
        $this->h_db->query('SELECT img_avatar FROM users WHERE id=:id');
        $this->h_db->bind(':id', $id);
        $row = $this->h_db->getSingleData();
        if($row){
            return $row->img_avatar;
        }
        else{
            return "no path foudn";
        }
        return $row;
    }
    public function getNumberArticleById($user_id){
        $this->h_db->query('SELECT * FROM posts WHERE user_id=:sql_req_id');
        $this->h_db->bind(':sql_req_id', $user_id);
        $this->h_db->execute();
        $number_art = $this->h_db->countRow();
        return $number_art;
    }
